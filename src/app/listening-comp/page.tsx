...
import { listeningPassages } from "@/data/comprehension";
import { useProgress } from "@/context/ProgressContext";
import { speakSpanish } from "@/lib/audio";

type Mode = "list" | "practice";

export default function ListeningCompPage() {
  ...
  const [phase, setPhase] = useState<"listen" | "questions" | "complete">("listen");
  const { addXP, recordListening } = useProgress();

  const passage = listeningPassages.find((p) => p.id === selectedId);
  ...
  function handlePlayAudio() {
    if (passage) speakSpanish(passage.text);
  }
        ...
        <main className="min-h-screen bg-neutral-950">
          <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in-up">
            ...
            <div className="rounded-2xl bg-white/5 border border-white/10 p-8 text-center mb-6">
              <p className="text-gray-400 mb-6">Listen carefully to the passage. You can replay it as many times as you need.</p>
              <button
                onClick={handlePlayAudio}
                className="rounded-xl bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 font-medium transition-colors text-lg inline-flex items-center gap-2"
              >
                <span>🔊</span>
    ...
    <main className="min-h-screen bg-neutral-950">
      <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in-up">
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-xl font-bold text-white">🎧 Listening Comprehension</h1>
          <div />
        </div>

        <p className="text-gray-400 mb-8 text-center">
          Listen to Spanish passages and answer comprehension questions!
        </p>
          ...
          {listeningPassages.map((p) => (
            ...
            >
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => speakSpanish(p.text)}
                  className="rounded-xl bg-white/10 hover:bg-white/15 text-white px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2"
                >
                  <span>🔊</span>
                  <span>Listen</span>
                </button>
                <button
                  ...
                  className="rounded-xl bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 text-sm font-medium transition-colors flex-1"
                >
                  Start →
                </button>
              </div>
